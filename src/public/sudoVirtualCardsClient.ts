...
import { ListProvisionalFundingSourcesUseCase } from '../private/domain/use-cases/fundingSource/listProvisionalFundingSourcesUseCase'
import { CancelProvisionalFundingSourceUseCase } from '../private/domain/use-cases/fundingSource/cancelProvisionalFundingSourceUseCase'
...
/**
 * Input for {@link SudoVirtualCardsClient.listProvisionalFundingSources}.
 *
...
export interface SudoVirtualCardsClient {
   ...
   *
   * @param {string} id The identifier of the provisional funding source to cancel.
   * @returns {ProvisionalFundingSource} The provisional funding source that was cancelled and is in a failed state.
   *
   * @throws {@link FundingSourceNotFoundError}
   *   No provisional funding source matching the specified ID could be found.
   */
  cancelProvisionalFundingSource(id: string): Promise<ProvisionalFundingSource>

  /**
   * Get a list of provisional funding sources. If no provisional funding
   * sources can be found, an empty list will be returned.
   *
   * @param {ListProvisionalFundingSourcesInput} input Parameters used to list the
   *        provisional funding sources.
   */
   ...
   *
   * @returns {ProvisionalVirtualCard} The card that is being provisioned. Please poll
   *          the card via the list methods to access the card.
   *
   * @throws {@link FundingSourceNotFoundError}
   *   No funding source matching the specified ID could be found.
...
export class DefaultSudoVirtualCardsClient implements SudoVirtualCardsClient {
  ...
  public async cancelProvisionalFundingSource(
    ...
    const cancelledProvisionalFundingSource = await useCase.execute(id)
    return ProvisionalFundingSourceApiTransformer.transformEntity(
      cancelledProvisionalFundingSource,
    )
  }
  ...
  public async listProvisionalFundingSources(
    ...
    const useCase = new ListProvisionalFundingSourcesUseCase(
      this.fundingSourceService,
      this.sudoUserService,
    )
    const { provisionalFundingSources, nextToken: resultNextToken } =
      await useCase.execute(input)
    const provisionalFundingSourceEntities = provisionalFundingSources.map(
      (item) => ProvisionalFundingSourceApiTransformer.transformEntity(item),
    )
    return {
      items: provisionalFundingSourceEntities,
      nextToken: resultNextToken,
    }
  }
